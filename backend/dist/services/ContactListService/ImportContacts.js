...
async function ImportContacts(contactListId, companyId, file) {
    ...
    const contacts = rows.map(row => {
        ...
        let number = "";
        let email = "";
        if ((0, lodash_2.has)(row, "nome") || (0, lodash_2.has)(row, "Nome") || (0, lodash_2.has)(row, "name")) {
    ...
    if (contactList) {
        for (let newContact of contactList) {
            try {
                ...
                console.log(e);
                logger_1.default.error(`Número de contato inválido: ${newContact.number}`);
            }
        }
    }
    return contactList;
