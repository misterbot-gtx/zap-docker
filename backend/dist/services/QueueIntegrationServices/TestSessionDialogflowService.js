...
const QueryDialogflow_1 = require("./QueryDialogflow");
const CreateSessionDialogflow_1 = require("./CreateSessionDialogflow");
const TestDialogflowSession = async ({ projectName, jsonContent, language }) => {
    ...
    const session = await (0, CreateSessionDialogflow_1.createDialogflowSession)(999, projectName, jsonContent);
    if (!session) {
        throw new AppError_1.default("ERR_TEST_SESSION_DIALOG", 400);
    }
    let dialogFlowReply = await (0, QueryDialogflow_1.queryDialogFlow)(session, projectName, "TestSession", "Ola", language, undefined);
    await session.close();
    if (!dialogFlowReply) {
