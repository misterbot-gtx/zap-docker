...
const ImportContactsService = async (companyId, whatsappId) => {
    const defaultWhatsapp = await (0, GetDefaultWhatsApp_1.default)(whatsappId, companyId);
        ...
        phoneContactsList.forEach(async ({ id, name, notify }) => {
            ...
            if (existingContact) {
                ...
                try {
                    ...
                    logger_1.default.warn(`Could not get whatsapp contacts from phone. Err: ${error}`);
                }
            }
        });
    }
