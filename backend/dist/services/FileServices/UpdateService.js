...
const UpdateService = async ({ fileData, id, companyId }) => {
    const file = await (0, ShowService_1.default)(id, companyId);
    ...
    if (options) {
        await Promise.all(options.map(async (info) => {
            ...
            // @ts-ignore
            await FilesOptions_1.default.upsert({ ...info, fileId: file.id });
        }));
        await Promise.all(file.options.map(async (oldInfo) => {
            const stillExists = options.findIndex(info => info.id === oldInfo.id);
            if (stillExists === -1) {
                await FilesOptions_1.default.destroy({ where: { id: oldInfo.id } });
            }
        }));
    }
    await file.update({
