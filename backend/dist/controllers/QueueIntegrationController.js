...
const TestSessionDialogflowService_1 = __importDefault(require("../services/QueueIntegrationServices/TestSessionDialogflowService"));
...
const store = async (req, res) => {
    const { type, name, projectName, jsonContent, language, urlN8N, typebotExpires, typebotKeywordFinish, typebotSlug, typebotUnknownMessage, typebotDelayMessage, typebotKeywordRestart, typebotRestartMessage } = req.body;
    const { companyId } = req.user;
...
const testSession = async (req, res) => {
    ...
    const { companyId } = req.user;
    const response = await (0, TestSessionDialogflowService_1.default)({ projectName, jsonContent, language });
    const io = (0, socket_1.getIO)();
    io.of(String(companyId))
        .emit(`company-${companyId}-queueIntegration`, {
        action: "testSession",
